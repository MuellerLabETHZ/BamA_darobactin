...
from Density_plot_Title import density_plot
from Optics import optics
from Pathway import pathway

from SplitByBoarders import splitbyboarders
from SplitByBoarders import assign_single_by_boarders

#%% User Input

#### Location of inputfiles
inputfolder = '/Users/rnoah/Desktop/DFS_Aligned/Aligned_Daro/1ums_D+'
# inputfolder = '/Volumes/Extreme SSD/DFS_Data/Daro/07ums/201014/Positive_Curves/peak_filtered'
# inputfolder = '/Users/rnoah/Desktop/BamA_TestData/lvm and tdms/txt_test_set'
...
if adjust_global_shift == True:
    ...
    # global_shift = -198.18969 #1ums_D+
    # global_shift = -178.99964 #3ums_D+
    # global_shift = -162.10903 #4.5ums_D+
    # global_shift = -148.41 #0.5ums_D+
    # global_shift = -170.848 #6ums_D+
    # global_shift = -216.385 #0.7ums_D+

    # global_shift = -103.073 #1ums_D-
    # global_shift = -69.5375 #3ums_D-
    # global_shift = -131.51 #4.5ums_D-
    # global_shift = -138.862 #0.5ums_D-
    # global_shift = -167.316 #6ums_D-
    # global_shift = -135.783 #0.7ums_D-
    
    # global_shift= shift_preview(FD_Curves, Space)
    print(condition, 'final global shift:', global_shift)
else:
    global_shift = 0
...
daro = from_title[-1]
Title = str(unfolding_speed) + ' um/s Unfolding Speed, ' + daro + 'Darobactin'

if do_density_plot == True:
    density_plot(FD_Curves, roi, bins_per_nm, Space, Title)
...
boarderclusters, noise_points, boarders = splitbyboarders(all_peaks_x, all_peaks_y, 5)
fig, ax = plt.subplots()
...
for cluster in boarderclusters:
    plt.title(Title)
    plt.plot(boarderclusters[cluster]['x'], boarderclusters[cluster]['y'],  '.', alpha=100/Nr_of_Selected)
    ax.set_xlabel('Contourlength [nm]')
...
pathway(boarderclusters, FD_Curves, Space)
fig, ax = plt.subplots()
for cluster in boarderclusters:
    plt.bar(cluster, len(boarderclusters[cluster]['x'])/Nr_of_Selected, color='b')
#%%

# #%% Optional: extract avg and stdev of clusters in x and y
#     clusterdata = np.zeros((len(clustering)-1, 5))
#     for cluster in clustering:
#         if cluster != -1:
#             x_avg = np.average(clustering[cluster]['x'])
#             x_std = np.std(clustering[cluster]['x'])
#             y_avg = np.average(clustering[cluster]['y'])
#             y_std = np.std(clustering[cluster]['y'])
